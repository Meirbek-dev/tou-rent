...
Фирменный стиль Торайгыров университета подключен так, как задумано
в архитектуре v3 § 4: заменой CSS-переменных в `src/styles/globals.css`,
компоненты не переписываются. Фирменной книги в репозитории нет - палитра
снята с переданных файлов знака (A-109).

- **Цвета**: синий `#1D3D66` и оранжевый `#DE6E35`. `--primary` - синий
  в светлой теме и светлый оттенок того же тона в темной: один токен годится
  и заливке кнопки, и тексту (гейт G17). Фиксированные брендовые поверхности -
  `bg-brand` / `text-brand-foreground` / `bg-brand-accent` - одинаковы
  в обеих темах (подвал).
- **Знак**: `public/brand/tou-emblem.png` (светлая тема) и
  `tou-emblem-white.png` (темная) в `components/app-logo.tsx`; подпись -
  текстом (`app_name`, `footer_university`), а не картинкой: лок-ап
  университета есть только белый (`tou-lockup-white.png`), он идет на синюю
  полосу подвала и на обложку Open Graph (`og-cover.png`, 1200×630).
- **Иконки и манифест**: `public/favicon.ico` (16/32/48),
  `public/brand/apple-touch-icon.png`, `icon-192.png`, `icon-512.png`;
  PWA-манифест - `public/manifest.json`. Все это отдается с корня сайта,
  в код не импортируется.
- **Абсолютные адреса** для `og:image`, `og:url` и canonical берутся из
  `src/lib/site.ts` (`SITE_ORIGIN`); значение обязано совпадать
  с `TOU_DOMAIN` в `infra/compose/.env`.
